 ...
 * 並將 Tile 2000-2007 寫入 src/data/maps/config.json
 *
...
const ground    = new Array(SIZE).fill(2003); // 預設：紅磚牆
...
// ─── (B) 北側建築正面 y=15：每 5 格交替 2004 烤鴨店 / 2005 中藥行 ────────────
// 地面 2003 + 建築正面 tile 取代 → collision 保持 1
for (let x = 2; x < W; x += 5) {
  const isEven = Math.floor((x - 2) / 5) % 2 === 0;
  ground[at(x, 15)] = isEven ? 2004 : 2005;
}
...
for (const [xL, xR] of ALLEYS) {
  for (let x = xL; x <= xR; x++) {
    // 巷道本體 y=23-27：髒污暗巷地磚（2002），可通行
    for (let y = 23; y <= 27; y++) {
      ground[at(x, y)]    = 2002;
      collision[at(x, y)] = 0;
    }
    // 巷底 y=28：垃圾堆（地面 2002 + 物件 2007），阻擋通行
    ground[at(x, 28)]  = 2002;
    objects[at(x, 28)] = 2007;
    // collision[at(x, 28)] 保持 1（阻擋）
  }
}

// ─── (D) 唐人街牌坊石柱：x=37，y=16（北人行道）與 y=22（南人行道）───────────
objects[at(37, 16)]   = 2006;
collision[at(37, 16)] = 1; // 石柱阻擋通行

objects[at(37, 22)]   = 2006;
...
// ─────────────────────────────────────────────────────────────────────────────
// 更新 config.json：新增 tile 2000-2007
// ─────────────────────────────────────────────────────────────────────────────
...
// 更新 _comment_groups
config._comment_groups.chinatown = '2000-2007 (哈卡街唐人街專用 tiles)';

fs.writeFileSync(configPath, JSON.stringify(config, null, 2));
console.log(`✓ config.json 更新 → 新增 ${added} 個 tile 定義（2000-2007）`);
